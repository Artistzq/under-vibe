...
如果你让一位计算机系的学生帮你配环境，他大概率会露出痛苦的表情。装 Java 要配 PATH，装 Python 发现有"两个"，`gcc` 报找不到，`node` 版本和项目对不上——一个晚上都在跟这些报错搏斗，代码一行还没写。好在 AI 出现了，他们终于可以从"兼职环境工程师"的岗位上解放出来——只是这份"被解放的痛"，现在轮到我们了。
