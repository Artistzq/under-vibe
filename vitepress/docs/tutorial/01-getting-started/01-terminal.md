...
在图形界面和 AI 时代到来之前，终端就是人类操控计算机的原生方式。即便到了今天，终端依然是程序员不可或缺的核心工具。本质上，用终端操控电脑和用鼠标点击图标操控电脑是**完全等价**的——一个用文字，一个用图形，只是交互方式不同。程序员的日常工作重度依赖终端，但对习惯了图形界面的人来说，那个黑窗口依然充满神秘感。那为什么我们需要理解终端？

<div class="outcome-grid">
  <div class="outcome-card">
    <span class="outcome-check">1</span>
    <div><strong>AI 时代的原生接口</strong><br>大语言模型本身就是纯文本输入输出，终端与 AI Agent 天然适配。</div>
  </div>
  <div class="outcome-card">
    <span class="outcome-check">2</span>
    <div><strong>程序员世界的入场券</strong><br>图形界面是为普通用户设计的。想站在程序员的视角看问题，终端是必须跨过的第一道门。</div>
  </div>
</div>

当然，未来也可能发生变化。如果大模型发展到可以直接理解屏幕截图、读取显卡输出并做出响应，人机交互方式可能会再次演变（比如 OpenClaw 这类 Agent 已经能解析截屏内容了）。

本章目标：学完后，你将能用准确的术语来描述终端——而不是"黑窗口"、"CMD"这些模糊的说法。你会知道 Terminal、Shell、命令行分别指的是什么。
...
| 操作系统 | 怎么打开 |
|-----|---------|
| macOS（苹果电脑） | 按 `Cmd + 空格` 打开聚焦搜索，输入 `Terminal` 回车 |
...
> **提示：** 在这个模拟终端里，`pwd`、`ls`、`cd`、`clear`、`mkdir`、`touch`、`cat`、`echo`、`rm` 都可以用。
> 随便试，不会弄坏任何东西！
...
| **Shell** | 终端将命令交给Shell程序，由Shell真正解释和执行命令 |
